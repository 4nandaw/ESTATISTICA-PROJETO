...
O intervalo de confiança (confidence interval) indica a faixa onde se espera que a média das previsões esteja, dado o modelo ajustado.

```{r}
predict(modelo4, dados_previsoes, interval = "confidence")
```

Para o primeiro conjunto de características (comprimento_nadadeira = 197.0, massa_corporal = 4202.0, sexo = macho, ilha = Biscoe):
-   O valor previsto (fit) é 17.91440.
-   O intervalo de confiança vai de 17.63222 a 18.19658, ou seja, estamos 95% confiantes de que a média da profundidade do bico para um grupo de pinguins com essas características está dentro desse intervalo..

Para o segundo conjunto de características (comprimento_nadadeira = 200.0, massa_corporal = 4100.0, sexo = fêmea, ilha = Dream):
-   O valor previsto (fit) é 16.66702.
-   O intervalo de confiança vai de 16.38806 a 16.94597, indicando que a média prevista para essa combinação de características está nesse intervalo.

## Intervalo de Previsão

O intervalo de previsão (prediction interval) é mais largo, pois indica onde se espera que valores individuais de novas observações possam cair, considerando a incerteza do modelo e a variabilidade dos dados.
...
Para o primeiro conjunto de características (comprimento_nadadeira = 197.0, massa_corporal = 4202.0, sexo = macho, ilha = Biscoe):
-   O valor previsto (fit) é 17.91440, mas o intervalo de previsão vai de 15.74398 a 20.08482, o que reflete a incerteza sobre onde exatamente a profundidade do bico de um pinguim individual com essas características pode estar.

Para o segundo conjunto de características (comprimento_nadadeira = 200.0, massa_corporal = 4100.0, sexo = fêmea, ilha = Dream):
-   O valor previsto (fit) é 16.66702, e o intervalo de previsão vai de 14.49701 a 18.83702, ou seja, a profundidade do bico para um pinguim individual pode cair em qualquer ponto dentro dessa faixa.


## Observações

  O intervalo de confiança é mais estreito e reflete a incerteza na média prevista para pinguins com essas características, enquanto o intervalo de previsão é mais amplo, pois reflete a incerteza nos valores individuais de profundidade do bico.
  
  Esses intervalos indicam que, para previsões individuais, você deve sempre considerar o intervalo de previsão, pois o intervalo de confiança seria muito estreito, aumentando o risco de não conter o valor real para um indivíduo específico.

# `Profundidade_bico` vs `Especie`

## Visualizações gráficas

```{r}

g1 <- ggplot(dados, aes(x=especie, y=profundidade_bico)) +
...
g2 <- ggplot(dados, aes(x=especie, y=profundidade_bico)) +
...
g3 <- ggplot(dados, aes(x=profundidade_bico, fill=especie)) +
